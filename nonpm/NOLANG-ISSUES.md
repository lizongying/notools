...
> **更新**: 问题 11（f64.to-str）、13（O-APPEND）已在当前编译器版本中修复。问题 14（结构体字段方法调用返回类型推断）已修复。问题 15（semver 测试）已修复。问题 16（json.parse segfault）已修复。

| 测试文件 | 状态 | 通过/失败 | 备注 |
|---------|------|----------|------|
| test-semver.no | ✅ 全部通过 | 41 PASS / 0 FAIL | 问题 14、15 已修复；所有测试通过 |
| test-utils.no | ✅ 全部通过 | PASS | 问题 13 已修复 |
| test-config.no | ✅ 全部通过 | PASS | 问题 13 已修复 |
| test-lockfile.no | ✅ 编译通过 | - | 问题 13 已修复 |
| test-json.no | ❌ LLVM 优化失败 | - | 问题 16 已修复（json.parse segfault 不再发生）；但仍存在 get-num 返回值代码生成问题 |
...
| 15 (semver 测试失败) | `compare-pre-release` 使用 `it` 提取 option 值；`max-satisfying` 使用 `to-string(v)` 规范化输出 | `nonpm/src/semver.no` |
| 16 (json.parse segfault) | `resolveModuleCallsInExpr` 增加 std struct 方法检查：当 `module.fn` 对应标准库方法 `module.module.fn` 时不改写为裸名 | `src/checker/checker.go` ~L4713 |
| 17 (多返回值函数 void call 误报警告) | 诊断条件增加 `m == 1` 检查，跳过多返回值函数 | `src/build/llvm/expr.go` ~L399 |

## 待解决问题

| 问题 | 严重程度 | 描述 |
|------|---------|------|
| 18 (新) | 高 | LLVM IR `multiple definition of local value named 'entry'`：函数内多次声明同名 alloca 变量 | test-workspace.no |
| 19 (新) | 高 | LLVM IR `trunc i64 to i1` 缺少操作数：条件分支生成空操作数 | test-linker.no |
| 20 (新) | 高 | `%str-long` vs `%vec` 类型不匹配：函数返回值类型推断错误 | test-tarball.no |
| 21 (新) | 高 | `%option` vs `%json.json-value` 类型不匹配：option 类型未正确窄化 | test-resolver.no, test-registry.no, test-installer.no |
| 22 (新) | 高 | `ptrtoint i8* 0 to i64`：空指针常量被直接用于 `ptrtoint` 指令 | test-run.no |
| 23 (新) | 高 | `i64` vs `ptr` 类型不匹配：str-long 指针操作中变量被错误推断为 `i64` 而非 `ptr` | test-publish.no |
