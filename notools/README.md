...
Unix 常用命令行工具集，使用 [Nolang](https://github.com/lizongying/nolang) 语言实现。193 个常用 Unix 命令，纯 Nolang 编写，不依赖外部系统命令。
...
- 支持 stdin 管道与文件输入
- 友好的错误处理
- 纯 Nolang 哈希算法实现（MD5、SHA-1/224/256/384/512、BLAKE2b、HMAC）
- 纯 Nolang 压缩算法实现（bzip2、xz/lzma、zstd、LZW .Z）
- 纯 Nolang 网络工具（HTTP 客户端 curl、ICMP ping）
...
> `md5`/`sha*` 为 BSD 风格命名，`md5sum`/`sha*sum` 为 GNU 风格命名，两者输出相同。
...
| 命令 | 说明 | 示例 |
|------|------|------|
| `md5` | MD5 摘要（128 位，纯 Nolang 实现） | `notools md5 file.txt` |
...
| `base64` | Base64 编码/解码（RFC 4648，`-d` 解码） | `notools base64 file.bin` |
...
# 哈希算法验证
echo -n abc | notools md5       # 900150983cd24fb0d6963f7d28e17f72
echo -n abc | notools sha256    # ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad
```
